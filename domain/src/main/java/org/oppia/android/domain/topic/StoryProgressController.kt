...
class StoryProgressController @Inject constructor(
  ...
  /**
   * Records the currently playing chapter of a specified exploration, story, topic as saved.
   * Returns a [DataProvider] that provides exactly one [AsyncResult] to indicate whether
   * this operation has succeeded. This method will never return a pending result.
   *
   * @param profileId the ID corresponding to the profile for which progress needs to be stored.
   * @param topicId the ID corresponding to the topic for which progress needs to be stored.
   ...
   * @return a [DataProvider] that indicates the success/failure of this record progress operation.
   */
  fun recordStartedNotCompletedProgressSavedChapter(
    profileId: ProfileId,
    topicId: String,
    storyId: String,
    explorationId: String,
    ...
    val deferred =
      retrieveCacheStore(profileId).storeDataWithCustomChannelAsync(
        ...
        val chapterProgressBuilder = if (previousChapterProgress != null) {
          ...
          ChapterProgress.newBuilder()
            .setChapterPlayState(ChapterPlayState.STARTED_NOT_COMPLETED_PROGRESS_SAVED)
            .setExplorationId(explorationId)
        }
        if (previousChapterProgress != null) {
          chapterProgressBuilder.lastPlayedTimestamp =
  ...
  /**
   * Records the currently playing chapter of a specified exploration, story, topic as not saved.
   * Returns a [DataProvider] that provides exactly one [AsyncResult] to indicate whether
   * this operation has succeeded. This method will never return a pending result.
   *
   * @param profileId the ID corresponding to the profile for which progress needs to be stored.
   ...
   * @return a [DataProvider] that indicates the success/failure of this record progress operation.
   */
  fun recordStartedNotCompletedNotProgressSavedChapter(
    profileId: ProfileId,
    topicId: String,
    storyId: String,
    explorationId: String,
    ...
    val deferred =
      retrieveCacheStore(profileId).storeDataWithCustomChannelAsync(
        ...
        val chapterProgressBuilder = if (previousChapterProgress != null) {
          ...
          ChapterProgress.newBuilder()
            .setChapterPlayState(ChapterPlayState.STARTED_NOT_COMPLETED_PROGRESS_NOT_SAVED)
            .setExplorationId(explorationId)
        }
        if (previousChapterProgress != null) {
          chapterProgressBuilder.lastPlayedTimestamp =
